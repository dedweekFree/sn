...
String custpk = request.getParameter("custpk");
...
<body>
<form name="form1" method="post" action="/ScImpServlet?formname=<%=formname%>&myframeurl=<%=myframeurl%>&picurl=<%=picurl%>&picsize=<%=picsize%>&pix=<%=pix%>&type=<%=type%>&tendtypeid=<%=tendtypeid%>&billno=<%=billno%>&custpk=<%=custpk%>&pctype=<%=pctype%>" enctype="multipart/form-data" onsubmit="return chkfrom();">
