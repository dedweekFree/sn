...
<%@ page import = "com.hitrust.b2b.trustpay.client.*,com.tend.servlet.*,java.sql.*" %>
...
String tMerchantRemarks          = request.getParameter("MerchantRemarks"      );
        tMerchantRemarks = new String(tMerchantRemarks.getBytes("ISO-8859-1"),"gb2312");
...
//5、直接支付请求提交成功,将客户端导向出示买方企业客户证书页面（下面注释的4行程序的参数值商户仍然可以取到）
  ...
  try {
	  ...
	  String sql = "insert into sn_abcbank_custpay(orderno,USERNAME,GETACCOUNTNAME,GETACCOUNT,GETACCOUNTNO,REMARK,TRNXAMOUNT)";
	  sql += " values('"+tMerchantTrnxNo+"','"+custcode+"','郑州思念食品有限公司','"+tAccountDBNo+"','"+tAccountDBBank+"','"+tMerchantRemarks+"','"+tTrnxAmount+"')";
	  stmt.addBatch(sql);
  ...
  System.out.println(tTrxResponse.getValue("PaymentURL"));
}
else {
...
%>
<a href='custpay.aspx'>返回</a></CENTER>
</BODY></HTML>
<%
}else{
%>
