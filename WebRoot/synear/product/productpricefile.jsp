...
<body>
<form name="form1" method="post" action="/ProductPriceServlet?formname=<%=formname%>&myframeurl=<%=myframeurl%>&picurl=<%=picurl%>&picsize=<%=picsize%>&pix=<%=pix%>&type=<%=type%>&tendtypeid=<%=tendtypeid%>&startdate=<%=startdate%>&remark=<%=remark%>&makerID=<%=makerID%>" enctype="multipart/form-data" onsubmit="return chkfrom();">
