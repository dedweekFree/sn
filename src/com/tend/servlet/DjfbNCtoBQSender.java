 ...
 *
 */
public class DjfbNCtoBQSender extends BaseDao implements Runnable {

	public DjfbNCtoBQSender() {
		System.out.println("单据辅表数据增量抽取--无参构造函数");
	}
	...
	public void run() {
		try {
			DateLoop("2012-11-01", "2013-01-01",3);
			System.out.println("单据辅表数据增量抽取完成");
		} catch (Exception e) {
			System.out.println("单据辅表抽取数据异常");
			e.printStackTrace();
		}
	}
	/**
	...
	public void NCtoBQ(String start,String end) throws Exception {
		...
		System.out.println("单据辅表数据增量抽取开始................");
		...
		try {
			...
			sql.append(" where f.dr=0");
			sql.append(" and  f.vouchid in (select z.vouchid");
			sql.append("                    from arap_djzb z");
			sql.append("                    where z.dr=0");
			...
			sql.append("                    and z.ts>='").append(start+"'");
			...
			while(restNC.next()){
				...
				insetSql.append("  insert into dwt_arap_djfb (");
				...
				insetSql.append("  ZYX9 ) values ( ");
					...
					try {
						...
						if(!result){
							System.out.println("第"+tm+"条F保存成功");
						}else{
							System.out.println("第"+tm+"条F保存失败");
						}
					} catch (Exception e) {
						e.printStackTrace();
					}finally {
			...
			System.out.println("单据辅表数据增量抽取完毕");
