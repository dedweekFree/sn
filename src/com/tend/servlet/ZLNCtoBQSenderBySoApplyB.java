 ...
 *
 */
public class ZLNCtoBQSenderBySoApplyB extends BaseDao implements Runnable {

	public ZLNCtoBQSenderBySoApplyB() {
	...
	public void DateLoop(String dbilldate,String begindate,String enddate,int days) throws Exception{
		...
		System.out.println("退货申请单辅表时间区间："+sdf.format(date1)+" to "+sdf.format(datetemp));
		...
		while(mm.dateCompare(datetemp , date2)){
			...
			if(mm.dateCompare(date2  ,datetemp )){
				System.out.println("退货申请单辅表时间区间："+sdf.format(mm.getDateAfterDay(datetemp, -days))+" to "+sdf.format(date2));
				NCtoBQ(dbilldate,sdf.format(mm.getDateAfterDay(datetemp, -days)), sdf.format(date2));
			}else{
				System.out.println("退货申请单辅表时间区间："+sdf.format(mm.getDateAfterDay(datetemp, -days))+" to "+sdf.format(datetemp));
